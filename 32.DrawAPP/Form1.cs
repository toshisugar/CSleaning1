...
namespace _32.DrawAPP
{
    public partial class Form1 : Form
    {
        ...
        private void DrawFigures(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;  //左辺さえ知っていれば、Shift+Alt+F10で、候補が出てくる。usingを使うか、右辺で呼び出すか。
            //円を描画する
        ...
        private void MousePressed(object sender, MouseEventArgs e)  //引数「MouseEventArgs a」。
            //このeにはとてもたくさんのマウスデータが入っている。このeにはとてもたくさんのマウスデータが入っている。位置やホイールを何回回したか等
        {
            ...
            //実は「this.startPosX = e.X;」とかで代入できる。
        }

        private void MouseDragged(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)  //e.Button(マウスボタン情報)が、MouseButton.Left(離した状態)にだったら
            {
                //終点座標を更新する
                this.endPos = new Point(e.X, e.Y);  //話した瞬間のe情報のX,Yを終点として記録する
